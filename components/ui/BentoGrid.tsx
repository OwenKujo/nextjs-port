  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: 'linear-gradient(90deg, rgba(2,16,42,1) 0%, rgba(2,23,68,1) 62%, rgba(0,91,103,1) 100%)'
      }}
    >
      <div className={`${id===6 && 'flex justify-center'} h-full`}>
        ...
        <div className={cn(
            titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-9'
        )}>
        ...
        {id === 3 &&(
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            <div className="flex flex-col gap-3 lg:gap-3">
              {[ 'Cloud platform', 'Full-stack dev', 'Artifical Intelligence'].map((item) => (
              <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-l opacity-50 lg:opacity-50 rounded-lg text-center bg-[#2c2f4c]">
                {item}
              </span>))}
            ...
            <div className="flex flex-col gap-3 lg:gap-3">
              <span className="py-4 px-3 rounded-lg text-center bg-[#2c2f4c]"/>
              {['Data science', 'Public speaking', 'Mentoring'].map((item) => (
              <span key={item} className="py-2 lg:py-4 lg:px-3 px-3 text-xs lg:text-l opacity-50 lg:opacity-50 rounded-lg text-center bg-[#2c2f4c]">
                {item}
              </span>
            ))}
